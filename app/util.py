...
def add_token_to_database(encoded_token: str):
  ...
  db_token = TokenBlacklist(
      jti=jti, token_type=token_type, expires=expires, revoked=revoked)
  db_token.save_new()
...
def revoke_token(raw_token: dict):
  ...
  try:
    ...
    raise TokenNotFound("Could not find the token {}".format(encoded_token))
...
def square_get_full_catalog() -> dict:
  ...
  def __associate_item_data_with_category_data(items: List[CatalogObject],
                                               categories: List[CatalogObject]
                                              ) -> List[CatalogObject]:
    '''
  ...
  itms_only = list(filter(lambda obj: obj.type == 'ITEM', itms_imgs_and_cats))
  imgs_only = list(filter(lambda obj: obj.type == 'IMAGE', itms_imgs_and_cats))
  cats_only = list(
      filter(lambda obj: obj.type == 'CATEGORY', itms_imgs_and_cats))
  itms_w_imgs = __associate_item_data_with_image_data_url(itms_only, imgs_only)
  itms_w_imgs_w_cats = __associate_item_data_with_category_data(
      itms_w_imgs, cats_only)
  return {
      'msg': 'Items retrieved',
      'items': [
          __catalog_item_to_dict(item.item_data) for item in itms_w_imgs_w_cats
      ],  # List[CatalogItem] w/ added category_data attr
      'categories': [cat.category_data.to_dict() for cat in cats_only
...
def square_get_checkout_url(cart: dict, square_customer_id: str) -> dict:
  ...
  for item in cart['items']:
    ...
    taxes = [
        OrderLineItemTax(catalog_object_id=tax_id) for tax_id in item['tax_ids']
    ]
    line_item = OrderLineItem(
        quantity=quantity, catalog_object_id=catalog_object_id, taxes=taxes)
  ...
  order_request = CreateOrderRequest(
      order=order, idempotency_key=gen_idem_key())
  ...
  api_instance.api_client.configuration.access_token = SQUARE_ACCESS_TOKEN
  url = api_instance.create_checkout(
      location_id=SQUARE_LOCATION_ID, body=body).checkout.checkout_page_url

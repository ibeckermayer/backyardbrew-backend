...
def test_no_jwt(sess_client: FlaskClient):
    '''test attempt to get account resource without jwt'''
    response = sess_client.get(ENDPOINT)
...
def test_jwt_access_valid(sess_client: FlaskClient,
    ...
    response = sess_client.get(ENDPOINT, headers=header)
